...
The Leg class can only work with coordinates in the local coordinate system of the leg, while the motion planning takes place in the global or base coordinate system. Therefore we need to transform between the coordinate frames. This can be done in multiple ways, for example using rotation matrices and shifting of the planes. 

To calculate the new end point of a leg in the local coordinate frame resulting from a translation along one of the axis of the base coordinate frame, we first need to shift and rotate the local frame to match the frame in the center of mass. To keep the code simple, an array is used to store the relative distances and angles between the local coordinate frame of each leg and the base coordinate system. This array (<code>legCoords[][]</code>) is specified in [Config.h](Config.h).
